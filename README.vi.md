# Bản Dịch Tiếng Việt Prison Architect
...
Bản dịch tiếng Việt hoàn chỉnh **100% Full Game** dành cho tựa game mô phỏng quản lý nhà tù nổi tiếng **Prison Architect 2D** trên Steam. Dự án dịch thuật trọn vẹn toàn bộ nội dung cốt lõi của game, tất cả các bản mở rộng (DLCs) và các chế độ chơi.
...
* **Font chữ Việt hóa đồng bộ**: Vẽ mới và thay thế bộ font Latinh bằng font chữ sans-serif mượt m mà, hỗ trợ đầy đủ 2402 ký tự giúp giao diện game sạch đẹp, sắc nét, không bị răng cưa hay lỗi đứt nét dấu tiếng Việt.
...
## ⚠️ Hướng dẫn sửa lỗi ngược dấu `à` / `á` khi chơi game
Do một lỗi kết xuất cố hữu trong Engine của game Prison Architect đối với các ngôn ngữ đặc thù (như tiếng Việt), ký tự **`à`** (dấu huyền) và **`á`** (dấu sắc) bị hiển thị hoán đổi (ngược) dấu cho nhau ở các văn bản hiển thị trên giao diện (ví dụ: *nhà* hiển thị dấu sắc, *thoát* hiển thị dấu huyền). Các nguyên âm khác như `è/é`, `ò/ó`, `ù/ú` vẫn hiển thị đúng hoàn toàn.

Để khắc phục triệt để và hiển thị đúng dấu 100% khi vào game, bạn chỉ cần thực hiện bước sau:

1. Mở Terminal (CMD / PowerShell) tại thư mục Mod:
   ```text
   %localappdata%\Introversion\Prison Architect\mods\VietnameseTranslation
   ```
2. Chạy lệnh sau trước khi mở game chơi để tự động sửa lỗi dấu:
   ```bash
