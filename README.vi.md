...
## ⚠️ Lưu ý về lỗi ngược dấu (`à` / `á`) từ Engine game
Do một lỗi kết xuất cố hữu trong Engine của game Prison Architect đối với các ký tự bitmap, ký tự **`à`** (dấu huyền) và **`á`** (dấu sắc) có thể hiếm khi bị hiển thị hoán đổi (ngược) dấu cho nhau ở một số văn bản nhất định trên giao diện (ví dụ: chữ *nhà* hiển thị dấu sắc, *thoát* hiển thị dấu huyền). Các nguyên âm khác như `è/é`, `ò/ó`, `ù/ú` vẫn hiển thị đúng hoàn toàn. Đây là lỗi kết xuất từ engine game gốc và hoàn toàn không ảnh hưởng đến trải nghiệm chơi game.
